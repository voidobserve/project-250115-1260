...
void TIMR0_IRQHandler(void) interrupt TMR0_IRQn
{
    ...
    // 周期中断
    if (TMR0_CONH & TMR_PRD_PND(0x1)) // 每1ms进入一次
    {
        TMR0_CONH |= TMR_PRD_PND(0x1); // 清除pending

        if (temperature_over_heat_cnt < 4294967295) // 防止计数溢出 
        {
            if (TEMPERATURE_STATUS_OVER_HEAT_0MIN == cur_temperature_status)
            {
                // 如果检测到温度过热，才累计时间
                temperature_over_heat_cnt++;
            }
            else
            {
                temperature_over_heat_cnt = 0;
            }
        }
